...
public class ManageOffCodes extends Menu {
    ...
    private void changeBeginDate (String code)  {
        try{command = inputInFormat("enter new begin date in format dd/MM/yyyy" , "^\\d{2}/\\d{2}/\\d{4}$");
    ...
    private void changeOffPercent (String code) {
        try{command = inputInFormat("please enter new off percent" , "\\d+");
            managerController.editDiscountCode(code,null,null,null,Integer.parseInt(command),null);}catch (Exception e){
            System.err.println(e.getMessage());
        }
    }
    ...
    private void createDiscountCode (){
        ...
        try {managerController.createDiscountCode(accounts , startDateString , endDateString , offPercentage , maxOff);
