...
public class RegisterSellerMenu extends Menu {
    ...
    String username, password, email, firstName, lastName, number, brand, balance;
    ...
    private void getBrand() {
        brand = inputInFormat("Please Enter Brand :", "\\w+");
        if (brand.equalsIgnoreCase("back")) getPassword();
        try {
            checkName(brand);
            new AddSellerRequest(new Seller(username, password, firstName, lastName,
                    Double.parseDouble(balance), email, number, brand));
            goToNextMenu();
        } catch (WrongFormatException e) {
            System.err.println(e.getMessage());
            getBrand();
